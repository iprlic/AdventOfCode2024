...
def get_border(i,j,cell, grid)
    border = []

    border.push({os: 'i', l: i-1, h: i, o: j, f: 'u'}) if i == 0 || grid[i-1][j] != cell
    border.push({os: 'i', l: i, h: i+1, o: j, f: 'd'}) if i == grid.length - 1 || grid[i+1][j] != cell
    border.push({os: 'j', l: j-1, h: j, o: i, f: 'l'}) if j == 0 || grid[i][j-1] != cell
    border.push({os: 'j', l: j, h: j+1, o: i, f: 'r'}) if j == grid[0].length - 1 || grid[i][j+1] != cell
...
def map_area(grid, i, j, cell, new_areas = [])
    ...
    border = get_border(i,j,cell,grid)
...
input.each_with_index do |row, a|
    row.each_with_index do |cell, b|
        ...
        borders = new_area.map{|area| area[:border]}.flatten(1)
       
        s = 0
        borders.group_by{|border| [border[:os], border[:l], border[:f]]}.each do |key, value|
            l = value.map{|v| v[:o]}.sort
            new_b = 1
            prev_l = l.shift
        ...
        total_price += s * new_area.size
        current_price = s * new_area.size
